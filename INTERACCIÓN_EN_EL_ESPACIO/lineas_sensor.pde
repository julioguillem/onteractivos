...
class Square {
  ...
  void update() {
    ...
    float b = map(0,0,100,400-distancia*4,width);
    square.beginDraw();
    ...
    square.rotate(r);
    square.fill(map(distancia*2, 0, 255, 230, 0));
    square.rect(0, 0, 200,2+b/20);
    square.popMatrix();
    square.endDraw();
    
    if (distancia <=300 && distancia >=172){
    ...
    if (distancia <=171 && distancia >=121){
